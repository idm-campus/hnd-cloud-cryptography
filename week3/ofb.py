##
# ofb.py
# A simple 3-bit OFB block cipher with ANSI X.923 like padding
#
# Ashen Gunaratne
# mail@ashenm.dev
#
...
def encrypt(key, iv, plaintext):
    ...
    for block in wrap(pad(plaintext, BLOCK_SIZE), BLOCK_SIZE):
        interim = nonce = cookbook[nonce]
        enc = xor(interim, block).rjust(BLOCK_SIZE, "0")
        results.append(enc)
...
def decrypt(key, iv, ciphertext):
    ...
    for block in wrap(ciphertext, BLOCK_SIZE):
        ...
        dnc, nonce = xor(interim, block).rjust(BLOCK_SIZE, "0"), interim
        results.append(dnc)
