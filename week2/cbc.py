##
# cbc.py
# A simple 3-bit CBC block cipher with ANSI X.923 like padding
#
# Ashen Gunaratne
# mail@ashenm.dev
#
...
from week2.ecb2 import pad, strip, swap
...
def encrypt(key, iv, plaintext):
    ...
    for block in wrap(pad(plaintext, BLOCK_SIZE), BLOCK_SIZE):
        interim = xor(block, nonce)
        lookup = interim.rjust(BLOCK_SIZE, "0")
        enc = nonce = cookbook[lookup]
        results.append(enc)
...
def decrypt(key, iv, ciphertext):
    ...
    for block in wrap(ciphertext, BLOCK_SIZE):
        lookup = cookbook[block]
        dnc = xor(lookup, nonce).rjust(BLOCK_SIZE, "0")
        nonce = block
        results.append(dnc)
